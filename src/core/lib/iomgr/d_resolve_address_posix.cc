...
static grpc_error* d_posix_blocking_resolve_address(
    const char* name, const char* default_port,
    grpc_resolved_addresses** addresses) {
puts("In d_posix_resolve_address in d_resolve_address_posix.cc");
  ...
  if (name[0] == 'u' && name[1] == 'n' && name[2] == 'i' && name[3] == 'x' &&
      name[4] == ':' && name[5] != 0) {
    return d_grpc_resolve_unix_domain_address(name + 5, addresses);
  }
...
static void d_do_request_thread(void* rp, grpc_error* /*error*/) {
...
static void d_posix_resolve_address(const char* name, const char* default_port,
                                  grpc_pollset_set* /*interested_parties*/,
                                  grpc_closure* on_done,
                                  grpc_resolved_addresses** addrs) {

  request* r = static_cast<request*>(gpr_malloc(sizeof(request)));
  GRPC_CLOSURE_INIT(&r->request_closure, d_do_request_thread, r, nullptr);
...
grpc_address_resolver_vtable d_grpc_posix_resolver_vtable = {
    d_posix_resolve_address, d_posix_blocking_resolve_address};
#endif
