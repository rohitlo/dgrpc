...
puts("tcp_server_create in tcp_server_posix.cc");
  grpc_tcp_server* s =
...
puts("on_read in tcp_server_posix.cc");
...
static grpc_error* add_wildcard_addrs_to_server(grpc_tcp_server* s,
                                                ...
                                                int requested_port,
                                                int* out_port) {
puts("add_wilcard_addr_to_server in tcp_server_posix.cc");
...
static grpc_error* tcp_server_add_port(grpc_tcp_server* s,
                                       ...
                                       int* out_port) {
puts("tcp_server_add_port in tcp_server_posix.cc");
  grpc_tcp_listener* sp;
...
puts("Before getsockname");
      if (0 == getsockname(sp->fd, reinterpret_cast<grpc_sockaddr*>(&sockname_temp.addr), &sockname_temp.len)) {
        int used_port = grpc_sockaddr_get_port(&sockname_temp);
        printf("used port %d",used_port);
        if (used_port > 0) {
...
puts("tcp_server_start in tcp_server_posix.cc");
  size_t i;
  grpc_tcp_listener* sp;
