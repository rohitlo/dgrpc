...
puts("grpc_set_socket_zerocopy in socket_utils_common_posix.cc");
...
static int create_socket(grpc_socket_factory* factory, int domain, int type,
                         int protocol) {
    puts("create_socket in sockets_util_common_posix.cc");
...
puts("grpc_create_dualstack_socket_using_factory in socket_utils_common_posix.cc");
  const grpc_sockaddr* addr =
      reinterpret_cast<const grpc_sockaddr*>(resolved_addr->addr);
  int family = addr->sa_family;
printf("Family in dualstack_sock %d\n",family);
  if (family == AF_INET6) {
    if (grpc_ipv6_loopback_available()) {
puts("grpc_create_dualstack_socket_using_factory ipv6 loopback");
      *newfd = create_socket(factory, family, type, protocol);
    } else {
puts("grpc_create_dualstack_socket_using_factory loopback else");
      *newfd = -1;
      errno = EAFNOSUPPORT;
    }
    /* Check if we've got a valid dualstack socket. */
    if (*newfd >= 0 && set_socket_dualstack(*newfd)) {
puts("grpc_create_dualstack_socket_using_factory set-socket_dualstack");
      *dsmode = GRPC_DSMODE_DUALSTACK;
      return GRPC_ERROR_NONE;
    }
    /* If this isn't an IPv4 address, then return whatever we've got. */
    if (!grpc_sockaddr_is_v4mapped(resolved_addr, nullptr)) {
puts("grpc_create_dualstack_socket_using_factory v4mapped");
      *dsmode = GRPC_DSMODE_IPV6;
      return error_for_fd(*newfd, resolved_addr);
    }
    /* Fall back to AF_INET. */
    if (*newfd >= 0) {
puts("grpc_create_dualstack_socket_using_factory >=0");
      close(*newfd);
    }
    family = AF_INET;
  }
