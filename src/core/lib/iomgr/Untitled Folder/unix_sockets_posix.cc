...
puts("grpc_create_socketpair_if_unix in unix_sockets_posix.cc");
  GPR_ASSERT(socketpair(AF_UNIX, SOCK_STREAM, 0, sv) == 0);
}

grpc_error* grpc_resolve_unix_domain_address(const char* name,
                                             grpc_resolved_addresses** addrs) {
    puts("grpc_resolve_unix_domain_address in unix_sockets_posix.cc");
...
int grpc_is_unix_socket(const grpc_resolved_address* resolved_addr) {
    puts("grpc_is_unix_socket in unix_sockets_posix.cc");
...
puts("grpc_unlink_if_unix_domain_socket in unix_sockets_posix.cc");
  const grpc_sockaddr* addr =
      reinterpret_cast<const grpc_sockaddr*>(resolved_addr->addr);
...
char* grpc_sockaddr_to_uri_unix_if_possible(
    const grpc_resolved_address* resolved_addr) {
puts("grpc_sockaddr_to_uri_unix_if_possible in unix_sockets_posix.cc");
  const grpc_sockaddr* addr =
      reinterpret_cast<const grpc_sockaddr*>(resolved_addr->addr);
  if (addr->sa_family != AF_UNIX) {
    return nullptr;
