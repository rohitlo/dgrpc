...
#include "src/core/lib/iomgr/d_socket_utils_posix.h"
#include "src/core/lib/iomgr/tcp_posix.h"
#include "src/core/lib/iomgr/tcp_server.h"
//#include "src/core/lib/iomgr/tcp_server_utils_posix.h"
#include "src/core/lib/iomgr/d_tcp_server_utils_posix.h"
//#include "src/core/lib/iomgr/unix_sockets_posix.h"
#include "src/core/lib/iomgr/d_unix_sockets_posix.h"

static grpc_error* d_tcp_server_create(grpc_closure* shutdown_complete,
                                     const grpc_channel_args* args,
                                     grpc_tcp_server** server) {
  grpc_tcp_server* s =
      static_cast<grpc_tcp_server*>(gpr_zalloc(sizeof(grpc_tcp_server)));
  s->so_reuseport = d_grpc_is_socket_reuse_port_supported();
...
static void d_finish_shutdown(grpc_tcp_server* s) {
  gpr_mu_lock(&s->mu);
...
static void d_destroyed_port(void* server, grpc_error* /*error*/) {
...
/* called when all listening endpoints have been shutdown, so no further
   ...
   things */
static void d_deactivated_all_ports(grpc_tcp_server* s) {
  ...
  if (s->head) {
    ...
    for (sp = s->head; sp; sp = sp->next) {
      d_grpc_unlink_if_unix_domain_socket(&sp->addr);
      GRPC_CLOSURE_INIT(&sp->destroyed_closure, d_destroyed_port, s,
...
static void d_tcp_server_destroy(grpc_tcp_server* s) {
  gpr_mu_lock(&s->mu);
  ...
  if (s->active_ports) {
    ...
    gpr_mu_unlock(&s->mu);
    d_deactivated_all_ports(s);
  }
}

/* event manager callback when reads are ready */
static void d_on_read(void* arg, grpc_error* err) {
  ...
  for (;;) {
    ...
    d_grpc_set_socket_no_sigpipe_if_possible(fd);
...
error:
  ...
  if (0 == --sp->server->active_ports && sp->server->shutdown) {
    gpr_mu_unlock(&sp->server->mu);
    d_deactivated_all_ports(sp->server);
  } else {
    gpr_mu_unlock(&sp->server->mu);
  }
}
...
static grpc_error* d_add_wildcard_addrs_to_server(grpc_tcp_server* s,
                                                unsigned port_index,
                                                int requested_port,
                                                int* out_port) {
  ...
  *out_port = -1;
  ...
  if ((v6_err = d_grpc_tcp_server_add_addr(s, &wild6, port_index, fd_index,
  ...
  if ((v4_err = d_grpc_tcp_server_add_addr(s, &wild4, port_index, fd_index,
...
static grpc_error* d_clone_port(grpc_tcp_listener* listener, unsigned count) {
  grpc_tcp_listener* sp = nullptr;
  char* addr_str;
  char* name;
  grpc_error* err;
  ...
  for (unsigned i = 0; i < count; i++) {
    ...
    int port = -1;
    grpc_dualstack_mode dsmode;
    err = d_grpc_create_dualstack_socket(&listener->addr, SOCK_STREAM, 0, &dsmode,
                                       &fd);
    if (err != GRPC_ERROR_NONE) return err;
    err = d_grpc_tcp_server_prepare_socket(listener->server, fd, &listener->addr,
                                         true, &port);
    if (err != GRPC_ERROR_NONE) return err;
...
static grpc_error* d_tcp_server_add_port(grpc_tcp_server* s,
  ...
  *out_port = -1;
  ...
  d_grpc_unlink_if_unix_domain_socket(addr);
  ...
  if (grpc_sockaddr_is_wildcard(addr, &requested_port)) {
    return d_add_wildcard_addrs_to_server(s, port_index, requested_port,
                                        out_port);
  }
  if (grpc_sockaddr_to_v4mapped(addr, &addr6_v4mapped)) {
    addr = &addr6_v4mapped;
  }
  if ((err = d_grpc_tcp_server_add_addr(s, addr, port_index, 0, &dsmode, &sp)) ==
      GRPC_ERROR_NONE) {
    *out_port = sp->port;
  }
  return err;
...
/* Return listener at port_index or NULL. Should only be called with s->mu
   locked. */
static grpc_tcp_listener* d_get_port_index(grpc_tcp_server* s,
                                         unsigned port_index) {
...
unsigned d_tcp_server_port_fd_count(grpc_tcp_server* s, unsigned port_index) {
  unsigned num_fds = 0;
  gpr_mu_lock(&s->mu);
  grpc_tcp_listener* sp = d_get_port_index(s, port_index);
...
static int d_tcp_server_port_fd(grpc_tcp_server* s, unsigned port_index,
                              unsigned fd_index) {
  gpr_mu_lock(&s->mu);
  grpc_tcp_listener* sp = d_get_port_index(s, port_index);
...
static void d_tcp_server_start(grpc_tcp_server* s, grpc_pollset** pollsets,
  ...
  sp = s->head;
  while (sp != nullptr) {
    if (s->so_reuseport && !d_grpc_is_unix_socket(&sp->addr) &&
        pollset_count > 1) {
      GPR_ASSERT(GRPC_LOG_IF_ERROR(
          "clone_port", d_clone_port(sp, (unsigned)(pollset_count - 1))));
      for (i = 0; i < pollset_count; i++) {
        grpc_pollset_add_fd(pollsets[i], sp->emfd);
        GRPC_CLOSURE_INIT(&sp->read_closure, d_on_read, sp,
                          grpc_schedule_on_exec_ctx);
        grpc_fd_notify_on_read(sp->emfd, &sp->read_closure);
        s->active_ports++;
        sp = sp->next;
...
grpc_tcp_server* d_tcp_server_ref(grpc_tcp_server* s) {
  gpr_ref_non_zero(&s->refs);
  return s;
}

static void d_tcp_server_shutdown_starting_add(grpc_tcp_server* s,
...
static void d_tcp_server_unref(grpc_tcp_server* s) {
  if (gpr_unref(&s->refs)) {
    grpc_tcp_server_shutdown_listeners(s);
    gpr_mu_lock(&s->mu);
    grpc_core::ExecCtx::RunList(DEBUG_LOCATION, &s->shutdown_starting);
    gpr_mu_unlock(&s->mu);
    d_tcp_server_destroy(s);
  }
}

static void d_tcp_server_shutdown_listeners(grpc_tcp_server* s) {
  gpr_mu_lock(&s->mu);
...
static grpc_core::TcpServerFdHandler* d_tcp_server_create_fd_handler(
    grpc_tcp_server* s) {
  s->fd_handler = new ExternalConnectionHandler(s);
  return s->fd_handler;
}

grpc_tcp_server_vtable d_grpc_posix_tcp_server_vtable = {
    d_tcp_server_create,        d_tcp_server_start,
    d_tcp_server_add_port,      d_tcp_server_create_fd_handler,
    d_tcp_server_port_fd_count, d_tcp_server_port_fd,
    d_tcp_server_ref,           d_tcp_server_shutdown_starting_add,
    d_tcp_server_unref,         d_tcp_server_shutdown_listeners};
