...
typedef struct grpc_tcp_server d_grpc_tcp_server;
...
} d_grpc_tcp_server_acceptor;

/* Called for newly connected TCP connections.
   Takes ownership of acceptor. */
typedef void (*d_grpc_tcp_server_cb)(void* arg, grpc_endpoint* ep,
...
class TcpServerFdHandler {
 public:
  ...
  virtual void d_Handle(int listener_fd, int fd,
...
typedef struct grpc_tcp_server_vtable {
  ...
  void (*start)(d_grpc_tcp_server* server, grpc_pollset** pollsets,
                size_t pollset_count, d_grpc_tcp_server_cb on_accept_cb,
                void* cb_arg);
  grpc_error* (*add_port)(d_grpc_tcp_server* s,
                          const grpc_resolved_address* addr,
                          int* out_port);
  grpc_core::TcpServerFdHandler* (*create_fd_handler)(d_grpc_tcp_server* s);
  unsigned (*port_fd_count)(d_grpc_tcp_server* s, unsigned port_index);
  int (*port_fd)(d_grpc_tcp_server* s, unsigned port_index, unsigned fd_index);
  d_grpc_tcp_server* (*ref)(d_grpc_tcp_server* s);
  void (*shutdown_starting_add)(d_grpc_tcp_server* s,
                                grpc_closure* shutdown_starting);
  void (*unref)(d_grpc_tcp_server* s);
  void (*shutdown_listeners)(d_grpc_tcp_server* s);
} d_grpc_tcp_server_vtable;
...
/* Number of fds at the given port_index, or 0 if port_index is out of
   bounds. */
unsigned d_grpc_tcp_server_port_fd_count(d_grpc_tcp_server* s, unsigned port_index);
...
int grpc_tcp_server_port_fd(d_grpc_tcp_server* s, unsigned port_index,
                            unsigned fd_index);

/* Ref s and return s. */
d_grpc_tcp_server* d_grpc_tcp_server_ref(d_grpc_tcp_server* s);
...
void d_grpc_tcp_server_global_init();
