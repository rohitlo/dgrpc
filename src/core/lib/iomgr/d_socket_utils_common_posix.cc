...
grpc_error* d_grpc_set_socket_zerocopy(int fd) {
...
grpc_error* d_grpc_set_socket_nonblocking(int fd, int non_blocking) {
...
grpc_error* d_grpc_set_socket_no_sigpipe_if_possible(int fd) {
...
grpc_error* d_grpc_set_socket_ip_pktinfo_if_possible(int fd) {
...
grpc_error* d_grpc_set_socket_ipv6_recvpktinfo_if_possible(int fd) {
  // Use conditionally-important parameter to avoid warning
  (void)fd;
...
grpc_error* d_grpc_set_socket_sndbuf(int fd, int buffer_size_bytes) {
...
grpc_error* d_grpc_set_socket_rcvbuf(int fd, int buffer_size_bytes) {
...
grpc_error* d_grpc_set_socket_cloexec(int fd, int close_on_exec) {
...
grpc_error* d_grpc_set_socket_reuse_addr(int fd, int reuse) {
  int val = (reuse != 0);
  int newval;
  socklen_t intlen = sizeof(newval);
...
/* set a socket to reuse old addresses */
grpc_error* d_grpc_set_socket_reuse_port(int fd, int reuse) {
#ifndef SO_REUSEPORT
...
void d_probe_so_reuseport_once(void) {
  ...
  if (s >= 0) {
    g_support_so_reuseport = GRPC_LOG_IF_ERROR(
        "check for SO_REUSEPORT", d_grpc_set_socket_reuse_port(s, 1));
    close(s);
  }
#endif
}

bool d_grpc_is_socket_reuse_port_supported() {
  gpr_once_init(&g_probe_so_reuesport_once, d_probe_so_reuseport_once);
  return g_support_so_reuseport;
}

/* disable nagle */
grpc_error* d_grpc_set_socket_low_latency(int fd, int low_latency) {
...
void d_config_default_tcp_user_timeout(bool enable, int timeout, bool is_client) {
  if (is_client) {
...
grpc_error* d_grpc_set_socket_tcp_user_timeout(
...
grpc_error* d_grpc_set_socket_with_mutator(int fd, grpc_socket_mutator* mutator) {
  GPR_ASSERT(mutator);
...
grpc_error* d_grpc_apply_socket_mutator_in_args(int fd,
  ...
  return d_grpc_set_socket_with_mutator(fd, mutator);
}

static gpr_once g_probe_ipv6_once = GPR_ONCE_INIT;
static int g_ipv6_loopback_available;

static void d_probe_ipv6_once(void) {
...
int d_grpc_ipv6_loopback_available(void) {
  gpr_once_init(&g_probe_ipv6_once, d_probe_ipv6_once);
...
/* This should be 0 in production, but it may be enabled for testing or
   ...
   also speak IPv4. */
int d_grpc_forbid_dualstack_sockets_for_testing = 0;

static int d_set_socket_dualstack(int fd) {
  if (!d_grpc_forbid_dualstack_sockets_for_testing) {
    const int off = 0;
...
grpc_error* d_grpc_create_dualstack_socket(
    const grpc_resolved_address* resolved_addr, int type, int protocol,
    grpc_dualstack_mode* dsmode, int* newfd) {
  return d_grpc_create_dualstack_socket_using_factory(
      nullptr, resolved_addr, type, protocol, dsmode, newfd);
}

static int d_create_socket(grpc_socket_factory* factory, int domain, int type,
                         int protocol) {
...
grpc_error* d_grpc_create_dualstack_socket_using_factory(
  ...
  if (family == AF_INET6) {
    if (d_grpc_ipv6_loopback_available()) {
      *newfd = d_create_socket(factory, family, type, protocol);
    } else {
      *newfd = -1;
      errno = EAFNOSUPPORT;
    }
    /* Check if we've got a valid dualstack socket. */
    if (*newfd >= 0 && d_set_socket_dualstack(*newfd)) {
...
uint16_t d_grpc_htons(uint16_t hostshort) { return htons(hostshort); }

uint16_t d_grpc_ntohs(uint16_t netshort) { return ntohs(netshort); }

uint32_t d_grpc_htonl(uint32_t hostlong) { return htonl(hostlong); }

uint32_t d_Pgrpc_ntohl(uint32_t netlong) { return ntohl(netlong); }

int d_grpc_inet_pton(int af, const char* src, void* dst) {
  return inet_pton(af, src, dst);
}

const char* d_grpc_inet_ntop(int af, const void* src, char* dst, size_t size) {
